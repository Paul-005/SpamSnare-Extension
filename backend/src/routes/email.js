...
authRouter.post('/generate-email', (req, res) => {
    ...
    const match = website.match(/(?:https?:\/\/)?(?:www\.)?(?:[a-z0-9-]+\.)*([a-z0-9-]+\.[a-z]{2,})(?=\/|$)/i);
    if (!match) {
    ...
    try {
        ...
        const selectStmt = conn.prepare(`SELECT "EMAIL" FROM USER_WEB WHERE "WEBSITE" = ?`);
