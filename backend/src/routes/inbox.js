...
inboxRoute.post('/check-flagged', async (req, res) => {
    ...
    if (!match) {
        return res.status(200).json({ flagged: false, message: 'Invalid website domain.' });
    }
    ...
    try {
        const flaggedSite = await prisma.flaggedSite.findUnique({
            where: { websiteAddress: domain }
        });
        if (!flaggedSite) {
